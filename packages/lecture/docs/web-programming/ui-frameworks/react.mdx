...
Aufrufen der Funktion bestehen bleibt. Diesesn Wert können wir dann  in das
...
HTML-Elemente müssen aktualisiert werden. Theoretisch könnte React die gesammte
Seite nehmen, alle Komponenten neu rendern und das gesamte HTML austauschen.

Das wäre aber sehr inperformant, hätte also hohen CPU-Bedarf und würde länger
dauern. Zudem müsste alle Event-Listener neu registriert werden, was die Performance
weiter senkt.
...
kann TypeScript vollständig prüfen, ob alle Pflichattribute gesetzt sind und
...
Ein weiterer Nachteil betsteht im Virtual DOM (VDOM). Auch wenn React bereits
