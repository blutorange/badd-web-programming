...
auch die ID eines Frames als `target` angeben, dies wird aber in der Praxis selten
verwendet.
...
`noreferrer noopener` gesezt. Weitere erlaubte Werte sind bei
[MDN web docs](https://developer.mozilla.org/en-US/docs/Web/HTML/Attributes/rel) 

:::info
Eine URL, auf die verlinkt wird, kann entweder absolut oder relativ sein. Eine
absolute URL ist etwa `http://example.com/web/content/index.html` oder
`http://example.com/web/images/logo.png`. Eine relative URL ist nur sinnvoll relativ
zu einer Basis-URL. Wenn wir uns zum Beispiel auf der Webseite `http://example.com/web/content/index.html`
...
das `<figure>`-Element zur Verfügung. Es zeichnet einen Teil aus als Illustration.
Wir können es etwa für verwenden für Bilder, Diagramme oder illustrative Code-Blöcke.
...
bessere Möglichkeiten, das Layout unserer Seite gemäßt unseren Wünschen zu gestalten.

:::note
Tatsächlich ist es aus Entwicklersicht eher schwieriger, Tabellen schön und gut
darzustellen und zu gestalten. Auch aus Sicht der Responiveness (Darstellung auf
Geräten mit kleiner Bildschirmbreite) müsen wir bei Tabellen entsprechend Testen
...
`<input>` ist ein einzeiliges Eingabfeld und das wohl am meisten verwendete Formularelement.
...
die Dev-Tools die Validationsregeln entfernen. Falls erforderlich, muss der Server,
...
Weitehrin können wir auch einzelne `<option>` in einem `<optgroup>`-Element schachteln,
...
um berechnete oder andersweitig abgeleitete Daten beim Absenden mit zu übertragen.
...
Entern, ist das Standarverhalten von Browsern, so vorzugehen, als ob der Submit-Button
...
der Webseite abgekappselt ist. Das betrifft sowohl das Design und Layout (CSS)
als auch JavaScript-Logik. Somit ist sichergestelt, dass die beiden verschiedenen
