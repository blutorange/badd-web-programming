...
uns Programmier frei vergebbar. Im JavaScript können wir auf diese Attribute
mittels der Eigenschaft `dataset` zugreifen.

Hier ist noch zu beachten, dass wir die Attributsnamen beim Zugriff über JavaScript
...
Mittels `outerHTML` können wir das HTML des Elements inklusive seiner Kindelemente
erhalten, mittels `innerHTML` das HTML von nur den Kindelementen:
