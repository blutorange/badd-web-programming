...
```js
// Aufgrund von technische Einschränkungen aktuell nicht in der Sandbox möglich
// Einfach einmal in der Konsole der Dev-Tools ausführen
console.log("a")
...
:::info
Wer obiges Beispiel ausprobiert, wird feststellen, dass `d` vor `c` ausgegeben
wird. Dies liegt daran, dass die Warteschlange noch einmal zwischen 2 Arten
von Tasks unterscheidet: Normale `Task` und
